...
function createEva() {
  ...
  return function eva() {
    var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
        ...
        _ref$onNotFound = _ref.onNotFound,
        onNotFound = _ref$onNotFound === undefined ? function (req, res, next) {
      ...
      next(err);
        ...
        onError = _ref$onError === undefined ? function (err, req, res, next) {
      throw err;
    } : _ref$onError,
    ...
    var app = {
      ...
      render: render,
      start: start,
      startWithSocket: startWithSocket,
    ...
    function init(_appInstance) {
      ...
      var appInstance = _appInstance || (0, _utils.createInstance)(MODE, LOGFILE, options);

      var appMongoose = db === null ? null : (0, _utils.connectMongodb)((0, _utils.createDbConf)(db[MODE] || db.DEFAULT || db), MODE);
    ...
    function use() {
      _instance.use.apply(_instance, arguments);
      return app;
    }
    ...
    function startWithSocket(serverInstance) {
      ...
      serverInstance.listen(server.PORT, server.HOST || 'localhost', function (err) {
        /* eslint-disable */
        if (err) {
          console.log(err);
        } else {
